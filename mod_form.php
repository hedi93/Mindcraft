...
/**
 * The main mindcraft configuration form
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_mindcraft_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('mindcraftname', 'mindcraft'), array('size' => '64'));
        ...
        $mform->setExpanded('settingsfieldset');

        // add the number of cards settings
        $options = array();
        for ($i=1; $i<=40; $i++) {
            $options[$i] = $i;
        }
        $mform->addElement('select', 'nummap', get_string('cardnumber', 'mindcraft'), $options);

        // add the interactive checkbox
